/**
 * Risk level mapping and standardized scan response builder.
 * Used across all scan services for consistent, UI-friendly output.
 */
...
const getSummary = (type, score) => {
  const labels = {
    url: {
      ...
      medium: 'This link is likely a phishing attempt.',
      high: 'This link is highly dangerous and likely a scam or phishing attack.',
    },
    message: {
      safe: 'This message appears to be genuine and safe.',
      low: 'This message has a few suspicious elements — stay alert.',
      medium: 'This message shows signs of being a scam or spam.',
    ...
    image: {
      ...
      high: 'This image shows strong indicators of fraudulent or scam content.',
    },
  };
  const tier = score <= 20 ? 'safe' : score <= 50 ? 'low' : score <= 75 ? 'medium' : 'high';
...
const getRecommendation = (type, score) => {
  if (score <= 20) return 'No suspicious patterns detected. You can proceed safely.';
  ...
  const recs = {
    ...
    message: {
      ...
      high: 'Do NOT respond, click links, or share any information. Block the sender and report as spam/scam.',
    },
    qr: {
    ...
    image: {
      ...
      high: 'This image is likely part of a scam. Do not share personal information or make any payments.',
    },
  };
  const tier = score <= 50 ? 'low' : score <= 75 ? 'medium' : 'high';
...
const buildScanResponse = ({ type, input, signals = [], explanation = [], details = {} }) => {
  ...
  return {
    ...
    signals: signals.map((s) => ({ type: s.type, weight: s.weight, description: s.detail })),
